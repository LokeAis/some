    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-100 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-sm text-neutral-900">Lynanalyse</h3>
        </div>
        <p className="text-sm text-neutral-800"><span className="font-medium">Hovudbodskap:</span> {result.analysis.main_message}</p>
        <p className="text-sm text-neutral-600 mt-1"><span className="font-medium text-neutral-800">Tonefall:</span> {result.analysis.tone}</p>
        {result.analysis.key_points?.length > 0 && (
          <ul className="mt-2 space-y-1">
            {result.analysis.key_points.map((p, i) => (
              <li key={i} className="text-sm text-neutral-600 flex gap-2"><span className="text-amber-500">•</span>{p}</li>
            ))}
          </ul>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {result.drafts.map((draft, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-sm border border-neutral-100 p-5 flex flex-col">
            ...
            <p className="text-sm text-neutral-800 whitespace-pre-wrap flex-grow">{draft.text}</p>
            ...
            <div className="mt-4 flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium border border-neutral-200 text-neutral-700 hover:bg-neutral-50 transition-colors"
              >
                {copiedIdx === idx
